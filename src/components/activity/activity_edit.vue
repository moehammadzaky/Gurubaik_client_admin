                                        ...
                                        <div v-if="activetab === 2" class="tabcontent">
                                            <form class="forms-sample" style="margin-top:30px" @submit.prevent="submitFile">
                                                    <div class="form-group">
                                                        ...
                                                        <label>Upload Dokumentasi</label><br>
                                                        <code>Pilih file untuk mengganti semua dokumentasi </code>
                                                        <input type="file" id="file" ref="file" accept="image/*,video/*" v-on:change="handleFileUpload()" class="form-control file-upload-info" style="padding-top:5px" multiple/>
                                                          ...
                                                          <div class="row" style="height:70px; overflow : hidden auto">
                                                            <div class=" col-lg-12" >
                                                                <div style="width:60px;height:60px;float:left;border: 1px solid #c1c1c1;margin:5px" v-for="(item,index) in dokumentasi" :key="index"> 
...
export default {
    ...
    methods:{
        getData(){
         this.$http.get('/activity/detail/'+this.$route.params.Id_activity).then(response => {
            let data = response.data.values
            var type = data.Activity_type.split(',')
            this.checkedType = type
            this.activity = data.Id_activity
            this.Activity_title = data.Activity_title
            this.Activity_desc = data.Activity_desk
            this.selectedJenjang = data.jenjang
            this.selectedKelas = data.kelas
            this.selectedMapel = data.mapel
            this.selectedKompetensi = data.kompetensi
            this.selectedTopik = data.topik
            ...
            for ( let a= 0 ; a < data.adb.length ; a++){
                ...
                //this.adb[a].name = data.adb[a].Activity_adb_name
                this.adb.push( 
                    {'count': data.adb[a].Acitivty_adb_count,
                    'name' : data.adb[a].Activity_adb_name
                    })
            }

            this.contentStep = data.step[0].Activity_step_text
            this.publish = data.Activity_publish
        ...
        submitFile(){
            ...
            if( !this.file ){
                ...
                this.$http.post( '/activity/dokumentasi/'+this.activity+'/'+true,formData,{
                    headers: {
        ...
        submitFileKerja(){
             ...
             if( !this.filekerja ){
                 ...
                 this.$http.post( '/activity/file/'+this.activity+'/'+true,formData,{
                    headers: {
                    'Content-Type': 'multipart/form-data'
                    }
                }).then(()  => { 
        ...
        submitAdb(){
                ...
                this.$http.post('/activity/adb/'+this.activity+'/'+true, this.adb
